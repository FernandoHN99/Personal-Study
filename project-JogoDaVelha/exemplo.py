...
def pontuar(matString, jogador):
    if (jogador * 4) in matString:
        return 4
    elif " " + (jogador * 3) in matString or (jogador * 3) + " " in matString:
        return 3
    elif (
        jogador + " " + (jogador * 2) in matString
        or (jogador * 2) + " " + jogador in matString
    ):
        return 2.5  # acréscimo
    elif " " + (jogador * 2) in matString or (jogador * 2) + " " in matString:
        return 2
    elif jogador + " " + jogador in matString or jogador + "  " + jogador in matString:
        return 1.5  # acréscimo
    else:
        return 0
...
def minimax(inicio, nivelDeDificuldade):
    _, coluna = maxNo(
        inicio, "X", nivelDeDificuldade
    )  # O computador quer maximizar seus pontos
    return coluna
...
def maxNo(inicio, vez, dificuldade):
    if dificuldade == 0:
        ...
        for i in range(len(sucessores)):
            if sucessores[i] == None:
                pontos.append(-math.inf)  # jogada impossível
            else:
