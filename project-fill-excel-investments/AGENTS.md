...
3. **HTTP nativo:** `QueryTables.Add(Connection:="URL;<url>")` em uma aba oculta de scratch (`_HttpScratch`). Funciona no Excel para macOS **e** Windows, sem shell/curl/MSXML.
...
6. **Estrutura modular:**
   ...
   - `modHttp`: `HttpGet(url)` via QueryTable + sheet oculta `_HttpScratch`.
...
5. Remover `_HttpScratch`, recalcular workbook, salvar.
...
- [x] **Etapa 2** – HTTP nativo via `QueryTables.Add("URL;...")` + `_HttpScratch`.
- [x] **Etapa 3** – Cotações (fetch + insert/update) em VBA puro.
- [x] **Etapa 4** – Investimentos (duplicar/inserir).
- [ ] **Etapa 5** – Importar `.bas`, criar botão e testar no Excel.
...
`modConfig.bas` → `modHttp.bas` → `modJson.bas` → `modTables.bas` → `modQuotes.bas` → `modInvestments.bas` → `modMain.bas`

**Setup no Excel (macOS):**
1. Abrir `01-Nando-Din.xlsm`; `Option+F11` (VBE).
2. `VBAProject` → botão direito → `Import File` para cada `.bas` na ordem acima.
...
- **HTTP via QueryTable:** cria/usa aba oculta `_HttpScratch` e a remove ao final. Não renomeie/use essa aba para outra coisa.
